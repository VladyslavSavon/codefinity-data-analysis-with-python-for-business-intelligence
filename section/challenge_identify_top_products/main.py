def top_products_by_revenue(summary_dict):
    ...
    top_3 = [name for name, revenue in sorted_products[:3]]
    return top_3

    sorted_products2 = sorted(summary_dict2.items(), key=lambda x: x[1], reverse=True)
    top_3 = [name for name, revenue in sorted_products2 [:3]]
    return top_3
